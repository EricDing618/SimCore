...
def demo():
    ...
    # 楼层高度特例
    first_floor = Floor(1, 5)
    building.floor_range[1] = first_floor

    # 创建电梯
    elevator1 = Elevator(eid=0, max_weight=1000, building=building)
    elevator2 = Elevator(eid=1, max_weight=1000, building=building)
    ...
    # 创建乘客
    # 创建乘客（使用整数楼层编号）
    passenger1 = Passenger(pid=1, weight=70, building=building, from_floor=1, to_floor=5, name='Peter', appear_time='2023/01/01 08:00:10')
    passenger2 = Passenger(pid=2, weight=80, building=building, from_floor=2, to_floor=6, name='Dick', appear_time='2023/01/01 08:00:20')

    # 添加乘客到大楼
    building.passengers = [passenger1, passenger2]

    for event in building.execute('FCFS'):
    ...
    while heap:
        ...
        match event_type_:
            ...
            case 'call_elevator':
                print(f"[{time_}] 乘客 {passenger_.name}(pid: {passenger_.pid}) 在楼层 {floor_.fid} 呼叫电梯 {building_.elevators}{passenger_.call_eid}（计划），目标楼层 {passenger_.to_floor}，质量 {passenger_.weight}kg")
